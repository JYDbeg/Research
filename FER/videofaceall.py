...
'''def tokutei(landmarks,img):
    ...
    le =(((np.sum(img[listy[0]:listy[1]+5,listx[0]:listx[1]+2],axis=2))/3)>100).flatten()
    re =(((np.sum(img[listy[2]:listy[3]+5,listx[2]:listx[3]+2],axis=2))/3)>100).flatten()
    mo = (((np.sum(img[listy[4]-30:listy[5]+5,listx[4]-4:listx[5]+2],axis=2))/3)>100).flatten()
    allfe = np.concatenate([le,re,mo])
    lenthg = len(allfe)
    shou = lenthg//224
    new_array = np.zeros((shou+1)*224)
    new_array[:lenthg] = allfe
    new_array = new_array.reshape((shou+1,224,1))
    return shou,new_array'''
'''def tokutei(landmarks,img):
    listx =[]
    listy = []
    for y in tokuteiy:
...
for name in names:
    for l in expr:
        model.clear_memory()
        path = f"D:/BaiduNetdiskDownload/py/OpenFace_2.2.0_win_x64/kyou/FEDB/{name}/{name}_{l}_0.mp4"
